...
import { employerApi, inviteApi, talentPoolApi, type Employer, type DashboardStats, type InterviewSession, type Job, type InviteTokenResponse, type TalentPoolCandidate, type Vertical, type RoleType } from '@/lib/api'
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-stone-500">{talentTotal} pre-interviewed candidates</p>
              </div>
            </div>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex flex-wrap gap-4 items-end">
                  <div className="w-40">
                    <Label className="text-xs text-stone-500 mb-1.5 block">Vertical</Label>
                    <select
                  ...
                  {talentVertical && availableTalentRoles.length > 0 && (
                    <div className="w-40">
                      <Label className="text-xs text-stone-500 mb-1.5 block">Role</Label>
                      <select
                        value={talentRole}
                  ...
                  <form onSubmit={handleTalentSearch} className="flex gap-2">
                    <Input
                      type="text"
                      ...
                      className="w-40 h-9"
                    />
                    <Button type="submit" size="sm" className="bg-teal-600 hover:bg-teal-700">Search</Button>
                  </form>
                </div>
              </CardContent>
            </Card>
            ...
            {talentLoading ? (
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {candidates.map((candidate) => {
                  const candidateId = candidate.profileId || candidate.candidateId
                  const displayName = candidate.candidateName || 'Unknown'
                  return (
                  <Card
                    key={candidateId}
                    className="hover:shadow-md transition-shadow cursor-pointer"
                    onClick={() => router.push(`/dashboard/talent-pool/${candidateId}`)}
                  >
                    <CardContent className="p-5">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-stone-500">
                            {candidate.roleType ? (ROLE_NAMES[candidate.roleType] || candidate.roleType) : 'No role specified'}
                          </p>
                          {candidate.university && (
                            <p className="text-xs text-stone-400 truncate mt-0.5">{candidate.university}</p>
                          )}
                        </div>
                        <div className="text-right flex-shrink-0">
                          <span className="text-lg font-semibold text-teal-600">{(candidate.bestScore ?? candidate.interviewScore ?? 0).toFixed(1)}</span>
                          <span className="text-xs text-stone-400">/10</span>
                        </div>
                      </div>
