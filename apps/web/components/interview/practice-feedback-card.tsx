...
function ScoreBar({ label, labelZh, score, maxScore = 10 }: { label: string; labelZh: string; score: number; maxScore?: number }) {
    ...
    <div className="space-y-1">
      <div className="flex justify-between items-center">
        <span className="text-sm text-slate-300">
          {label} <span className="text-slate-500">({labelZh})</span>
        </span>
...
export function PracticeFeedbackCard({
  ...
  const getOverallRating = () => {
    if (overallPercentage >= 80) return { label: 'Excellent', color: 'text-emerald-400' }
      ...
      <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-emerald-100 text-sm">Practice Feedback</p>
              <h2 className="text-xl font-semibold text-white">
                Question {questionNumber} of {totalQuestions}
              </h2>
            </div>
            <div className="text-right">
              <p className="text-emerald-100 text-sm">Overall Score</p>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 bg-slate-700/30 rounded-lg p-4">
              <ScoreBar label="Communication" labelZh="沟通能力" score={scoreDetails.communication} />
              <ScoreBar label="Problem Solving" labelZh="解决问题" score={scoreDetails.problemSolving} />
              <ScoreBar label="Domain Knowledge" labelZh="专业知识" score={scoreDetails.domainKnowledge} />
              <ScoreBar label="Motivation" labelZh="动机" score={scoreDetails.motivation} />
              <ScoreBar label="Culture Fit" labelZh="文化契合" score={scoreDetails.cultureFit} />
            </div>
          </div>
