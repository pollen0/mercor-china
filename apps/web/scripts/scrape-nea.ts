#!/usr/bin/env npx tsx
/**
 * Playwright scraper for NEA's Consider-powered careers portal.
 *
 * Usage:
 *   cd apps/web
 *
 *   # Discovery mode: dump rendered HTML to inspect DOM structure
 *   npx tsx scripts/scrape-nea.ts --discover
 *
 *   # Scrape mode: extract US intern jobs
 *   npx tsx scripts/scrape-nea.ts --output "../api/scripts/data/nea.json"
 *
 *   # Scrape with detail pages
 *   npx tsx scripts/scrape-nea.ts --output "../api/scripts/data/nea.json" --scrape-details
 *
 *   # Headed mode (see the browser)
 *   npx tsx scripts/scrape-nea.ts --output "../api/scripts/data/nea.json" --headed
 */
...
function parseArgs(): CliArgs {
  ...
  if (!parsed.discover && !parsed.output) {
    console.error("Usage: npx tsx scripts/scrape-nea.ts [OPTIONS]");
    console.error("");
    console.error("Modes:");
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// NEA Consider portal: navigate and wait for content
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const NEA_BOARD_URL = "https://careers.nea.com";
// Default: intern filter only (no jobType restriction to catch all verticals)
const NEA_INTERN_URL = "https://careers.nea.com/jobs?titlePrefix=intern";

async function navigateAndWait(page: Page): Promise<void> {
  console.log(`Navigating to ${NEA_INTERN_URL}...`);
  await page.goto(NEA_INTERN_URL, { waitUntil: "domcontentloaded", timeout: 60000 });
...
async function discoverDOM(page: Page): Promise<void> {
  ...
  // Take screenshot
  const screenshotPath = path.join(dumpDir, "nea-portal-screenshot.png");
  await page.screenshot({ path: screenshotPath, fullPage: true });
  console.log(`Screenshot saved to: ${screenshotPath}`);

  // Dump the rendered HTML
  const html = await page.content();
  const htmlPath = path.join(dumpDir, "nea-portal-rendered.html");
  ...
  const analysisPath = path.join(dumpDir, "nea-portal-analysis.json");
...
async function extractJobCards(page: Page): Promise<JobCard[]> {
  ...
  const jobs = await page.evaluate(() => {
    ...
    // Each company group is a .grouped-job-result
    const companyGroups = document.querySelectorAll(".grouped-job-result");

    companyGroups.forEach((group) => {
      // Extract company name from the header logo alt text: "CompanyName logo"
      ...
      // Company size: generic tag without a specific subclass
      let size = "";
  ...
  console.log(`Extracted ${jobs.length} jobs from ${new Set(jobs.map((j) => j.company)).size} companies.`);
  return jobs;
}
...
async function main() {
  ...
  console.log("NEA Careers Portal Scraper (Consider platform)");
  console.log(`  Board:  ${NEA_BOARD_URL}`);
  ...
  try {
    ...
    if (args.discover) {
      ...
      console.log("Then update extractJobCards() with the correct selectors and run in scrape mode.");
      return;
    }
